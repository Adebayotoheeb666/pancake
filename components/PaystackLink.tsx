...
const PaystackLink: React.FC<PaystackLinkProps> = ({
  user,
  variant = 'primary',
}) => {
  ...
  const handleVerifyAccount = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/bank/paystack/verify', {
        method: 'POST',
  ...
  const loadBanks = async () => {
    try {
      const response = await fetch('/api/bank/paystack/banks');
      const data = await response.json();
      setBanks(data.banks || []);
    } catch (error) {
    ...
    <>
      {variant === 'primary' ? (
        <Button
          ...
          className="paystacklink-primary"
        >
          Connect with Paystack
        </Button>
      ) : variant === 'ghost' ? (
        <Button onClick={() => setShowForm(true)} variant="ghost" className="paystacklink-ghost">
          <Image
        ...
        <Button onClick={() => setShowForm(true)} className="paystacklink-default">
          <Image
            src="/icons/connect-bank.svg"
            alt="connect bank"
            width={24}
