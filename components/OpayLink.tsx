...
const OpayLink: React.FC<OpayLinkProps> = ({
  user,
  variant = 'primary',
}) => {
  ...
  const handleVerifyAccount = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/bank/opay/verify', {
        method: 'POST',
  ...
  const loadBanks = async () => {
    try {
      const response = await fetch('/api/bank/opay/banks');
      const data = await response.json();
      setBanks(data.banks || []);
    } catch (error) {
    ...
    <>
      {variant === 'primary' ? (
        <Button
          ...
          className="opaylink-primary"
        >
          Connect with Opay
        </Button>
      ) : variant === 'ghost' ? (
        <Button onClick={() => setShowForm(true)} variant="ghost" className="opaylink-ghost">
          <Image
        ...
        <Button onClick={() => setShowForm(true)} className="opaylink-default">
          <Image
            src="/icons/connect-bank.svg"
            alt="connect bank"
            width={24}
