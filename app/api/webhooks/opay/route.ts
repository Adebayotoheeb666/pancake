...
export async function POST(request: NextRequest) {
  try {
    ...
    const secret = process.env.OPAY_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET || '';
    const header = request.headers.get('x-opay-signature') || request.headers.get('x-signature') || '';
    ...
    const external_transfer_id = body.data?.transactionId || body.transactionId || body.id;
    const status = body.status || body.data?.status || 'processing';

    const updates: any = { status, status_message: `OPay webhook: ${status}` };
    ...
    if (error) {
      ...
      if (e2) {
        ...
        console.error('OPay webhook update failed', e2);
