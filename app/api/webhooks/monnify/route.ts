...
export async function POST(request: NextRequest) {
  try {
    ...
    const secret = process.env.MONNIFY_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET || '';
    const header = request.headers.get('x-signature') || request.headers.get('x-monnify-signature') || '';
    ...
    const external_transfer_id = body.response?.transactionId || body.transactionId || body.id;
    const status = body.event || body.response?.status || 'processing';

    const updates: any = { status, status_message: `Monnify webhook: ${status}` };
    ...
    if (error) {
      ...
      if (e2) {
        ...
        console.error('Monnify webhook update failed', e2);
